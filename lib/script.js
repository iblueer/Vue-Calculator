  ...
  methods: {
    ...
    append(character) {
      ...
      if (this.isOperator(character) === false) {
        ...
        if (character === "." && this.isOperator(this.equation[this.equation.length - 1])) {
      ...
      if (this.isOperator(character) === true && this.isOperatorAdded !== true) {
    ...
    calculate() {
      ...
      // 【学习他人好榜样】让大于1.0e9的数字，用科学计数法展示，此处用到了三元运算符，最外面包了一层toString();
      this.equation = (answer < 1.0e9 ? parseFloat(answer.toFixed(9)) : answer.toExponential(3)).toString();
